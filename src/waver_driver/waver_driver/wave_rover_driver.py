...
class WaveRoverDriver(Node):
    ...
    def _lift(self, norm: float, deadband: float) -> float:
        """Apply dead-zone feedforward to one twist axis.

        Remaps a nonzero normalized command ([-1, 1]) onto
        [deadband, output_max] so the smallest command still overcomes friction
        on that axis. A near-zero command stays 0 (a true stop) rather than
        lurching to the dead-zone floor.
        """
        if abs(norm) < 1e-3:
            return 0.0
    ...
    def _ang_feedforward(self, wz: float, lin_c: float) -> float:
        """Compute open-loop angular PWM for a yaw-rate target.

        Normalizes the target, then lifts it out of the scrub dead-zone, which
        is itself washed out while the robot is rolling.
        """
